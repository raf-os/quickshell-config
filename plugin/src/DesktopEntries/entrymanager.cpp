...
DesktopEntryScanner::DesktopEntryScanner(DesktopEntryManager *manager)
    : m_manager(manager) {
...
void DesktopEntryScanner::run() {
  ...
  QMetaObject::invokeMethod(this->m_manager,
                            "onScanCompleted",
                            Qt::QueuedConnection,
                            Q_ARG(QList<EntryData>, results));
}
...
void DesktopEntryScanner::scanDirectory(const QString    &path,
  ...
  while (it.hasNext()) {
    ...
    if (entry.isDir()) {
      ...
      auto _data = DesktopEntryManager::parseText(id, entry.filePath());
...
DesktopEntryManager::DesktopEntryManager(QObject *parent) : QObject(parent) {}
...
DesktopEntryManager::parseText(const QString &id,
...
QStringList DesktopEntryManager::parseExecString(const QString &execString) {
...
void DesktopEntryManager::onScanCompleted(const QList<EntryData> &results) {
